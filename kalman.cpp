...
{
    measurement_img = Mat::zeros(3, 1, CV_64F);  // 测量矩阵
    kalman_img.transitionMatrix = (Mat_ < double > (3, 3) <<    // 转移矩阵
        1, runtime, runtime * runtime / 2, //下一步长的计算值为当前坐标加上当前速度乘以步长
        0, 1, runtime,
        0, 0, 1);
    kalman_img.processNoiseCov = (Mat_ < double > (3, 3) <<    // 雅可比矩阵
...
//    }
    if(temp_x <= SIZE_X && temp_x >= 0)  // 边界处理
    {
