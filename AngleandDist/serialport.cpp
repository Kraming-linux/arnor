...
char SerialPort::temp_write_buff[15];
char SerialPort::write_buff[19];
 ...
 * @param x 点的x坐标
 * @param y 点的y坐标
 * @param depth 深度:自瞄模式通过测距,大神符模式默认给7200
 */
 ...
 * @param _yaw : yaw轴角的正负
 * @param yaw  : yaw轴角度
 * @param _pitch: pitch轴角的正负
 * @param pitch : pitch轴角度
 * @param depth 深度:自瞄模式通过测距,大神符模式默认给7200
 */
void SerialPort::sendDataAngle(int model, int model_select, int _yaw, int yaw, int _pitch, int pitch, int depth){
    sprintf(temp_write_buff, "%c%01d%01d%01d%03d%01d%03d%04d", 'S', model, model_select, _yaw, yaw, _pitch, pitch, depth);
    uint8_t CRC_check = Check_CRC8(temp_write_buff, sizeof(temp_write_buff));
    sprintf(write_buff, "%c%01d%01d%01d%03d%01d%03d%04d%03d%c", 'S', model, model_select,_yaw, yaw, _pitch, pitch, depth, CRC_check, 'E');

    write(fd, write_buff, sizeof(write_buff));
    #if COUT_STATE == 1
    cout << "send_buff:" << write_buff << endl;
    #endif
    usleep(1);
}
